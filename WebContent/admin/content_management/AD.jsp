...
function confirmDelete(id) {
	...
	if(flag==true){
		$.post(basePath+"/ADAction",{type:"delete",id:id},function(result){
			if(result=='success'){
				location.reload();
			}
		})
					...
					<div class="btn-toolbar ls-btn-bar">
						<div class="btn-group">
							...
							<button class="btn btn-default" type="button"
								onclick="location.href='add_AD.jsp'">新增</button>
						</div>
						<div class="btn-group">
							...
							<c:forEach var="ad" items="${ads}">
								<tr>
									<td><input type="checkbox" name="ids"
										value="${ad.id}"></td>
									<td align="center"><a
										href="${basePath}/admin/content_management/update_AD.jsp?id=${ad.id}" class="ls-opt">修改</a>
										<a href="#" onclick="confirmDelete('${ad.id}');"
										class="ls-opt">删除</a></td>
									<td>${ad.id}</td>
									<td>
										<div>
											<a href="#" title="${ad.title}">${ad.title}</a>
										</div>
									</td>
									<td>
										<div>${ad.date}</div>
